...
const (
	...
	RESPONSE_NOT_FOUND_REQUEST_CODE     = 404
	...
	RESPONSE_NOT_FOUND_REQUEST_MSG      = "NOT FOUND ANY DEVICE"
	...
	RESPONSE_DDB_DATA_NOT_EXIST_MSG     = "DATA NOT EXIST EXCEPTION"
)
...
func Handler(request events.APIGatewayProxyRequest) (res events.APIGatewayProxyResponse, err error) {
	...
	if deviceId != "" {
		dev = loadFromDdb(deviceId)
	} else {
		panic(internalError{RESPONSE_NOT_FOUND_REQUEST_CODE, RESPONSE_NOT_FOUND_REQUEST_MSG})
	}
	...
	return getResponse(RESPONSE_OK_CODE, string(jsonStr)), nil
